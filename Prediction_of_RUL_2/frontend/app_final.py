...
if upload_data is not None:
    ...
    endpoint2='http://host.docker.internal:8000/m' # Specify this path for Dockerization to work
    if st.button('Predict'):
      
        res=requests.post(url= 'http://host.docker.internal:8000/predict',data=json.dumps(inputs))
        st. subheader(f"The RUL  of engine{en} is= {res.text}")
        mn=requests.post(url='http://host.docker.internal:8000/m',data=json.dumps(inputs))
